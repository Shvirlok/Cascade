...
export class CDCListenerService {
  ...
  public async handleWebhookPayload(payload: any): Promise<void> {
    ...
    if (updatedRecord && (updatedRecord.status === 'DELAYED' || updatedRecord.status === 'CANCELLED')) {
      ...
      await this.triggerAgentHealingDirectly(itineraryId, segmentId, delayMinutes, 'FLIGHT_DELAY', 'EXECUTIVE_SPEED', undefined, 'Executive Traveler');
    }
  }
  ...
  private async checkPendingDisruptions(): Promise<void> {
    try {
      ...
      for (const row of pendingRes.rows) {
        ...
        this.triggerAgentHealingDirectly(row.itinerary_id, row.segment_id, row.delay_minutes, 'FLIGHT_DELAY', 'EXECUTIVE_SPEED', undefined, 'Executive Traveler').catch(() => {});
      }
    } catch (_err) {
  ...
  public async triggerAgentHealingDirectly(
    ...
    const resolvedTravelerName = travelerName || 'Sarah Jenkins';
