...
export class CDCListenerService {
  ...
  private isExecutingCheck: boolean = false;
  ...
  /**
   * Start CDC event listener loop with controlled recursive timeout to prevent overlapping executions
   */
  public startListening(): void {
    ...
    const pollLoop = async () => {
      ...
      if (this.isPolling) {
        this.pollTimer = setTimeout(pollLoop, this.pollIntervalMs);
      }
    };
    pollLoop();
  }
  ...
  private async checkPendingDisruptions(): Promise<void> {
    if (this.isExecutingCheck) return;
    this.isExecutingCheck = true;
