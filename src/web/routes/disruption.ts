...
export function setDisruptionDeps(
  ...
  disruptionRouter.post('/api/disrupt', async (req: Request, res: Response) => {
    ...
    // Only mutate the shared in-memory graph safely if this disruption targets the default itinerary
    if ((itineraryId === 'itin-101' || itineraryId === inMemoryGraphCache?.itinerary?.id) && Array.isArray(inMemoryGraphCache?.segments)) {
    ...
    const targetItinId = itineraryId || inMemoryGraphCache?.itinerary?.id || 'itin-101';
    const targetSegId = (inMemoryGraphCache?.segments?.[0]?.id) || 'c2fbc999-7c0b-4ef8-bb6d-8bb9bd380a33';
